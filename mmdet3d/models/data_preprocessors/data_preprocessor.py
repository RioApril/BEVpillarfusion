...
@MODELS.register_module()
class ThreeViewDataPreprocessor(Det3DDataPreprocessor):
    """支持 BEV、XOZ、YOZ 三视图体素化的数据预处理器。"""

    def __init__(self,
                 bev_voxel_layer: dict,
                 xoz_voxel_layer: dict,
                 yoz_voxel_layer: dict,
                 **kwargs):
        super().__init__(voxel=False, **kwargs)
        self.bev_voxel_layer = VoxelizationByGridShape(**bev_voxel_layer)
        self.xoz_voxel_layer = VoxelizationByGridShape(**xoz_voxel_layer)
        self.yoz_voxel_layer = VoxelizationByGridShape(**yoz_voxel_layer)

    def simple_process(self, data: dict, training: bool = False) -> dict:
        data = super().simple_process(data, training)
        batch_inputs = data['inputs']
        if 'points' not in batch_inputs:
            return data

        points_list = batch_inputs['points']
        # BEV
        bev_voxels, bev_coors, bev_num_points = self._voxelize_points(
            points_list, self.bev_voxel_layer)
        # XOZ
        xoz_voxels, xoz_coors, xoz_num_points = self._voxelize_points(
            points_list, self.xoz_voxel_layer)
        # YOZ
        yoz_voxels, yoz_coors, yoz_num_points = self._voxelize_points(
            points_list, self.yoz_voxel_layer)

        batch_inputs.update({
            'bev_voxels': bev_voxels,
            'bev_num_points': bev_num_points,
            'bev_coors': bev_coors,
            'xoz_voxels': xoz_voxels,
            'xoz_num_points': xoz_num_points,
            'xoz_coors': xoz_coors,
            'yoz_voxels': yoz_voxels,
            'yoz_num_points': yoz_num_points,
            'yoz_coors': yoz_coors,
        })
        return data
    ...
    def _voxelize_points(self,
        ...
        for batch_idx, points in enumerate(points_list):
            ...
            batch_inds = torch.full((coors.shape[0], 1), batch_idx,
                                    dtype=coors.dtype, device=coors.device)
            coors = torch.cat([batch_inds, coors], dim=1)
...
@MODELS.register_module()
class MultiViewDataPreprocessor(Det3DDataPreprocessor):
    """
    支持双视图体素化的数据预处理器：
    - BEV视图：压缩z方向，保留x,y
    - 侧视图（xoz平面）：压缩y方向，保留x,z

    使用 VoxelizationByGridShape 进行体素化，与官方 Det3DDataPreprocessor 一致。
    """
    ...
    def __init__(self,
                 ...
                 **kwargs):
        # 调用父类，禁用其体素化功能（因为我们要自己处理两个视图）
        super().__init__(voxel=False, **kwargs)

        # 创建两个体素化层，直接传入配置字典
        self.bev_voxel_layer = VoxelizationByGridShape(**bev_voxel_layer)
        self.side_voxel_layer = VoxelizationByGridShape(**side_voxel_layer)

    def forward(self, data: Union[dict, list], training: bool = False):
        """重写 forward，同时生成 BEV 和 侧视图 的体素"""
        if isinstance(data, list):
            # 测试时增强（TTA）
            aug_batch_data = []
            for single_data in data:
                aug_batch_data.append(self.simple_process(single_data, training))
            return aug_batch_data
        else:
            return self.simple_process(data, training)
    ...
    def simple_process(self, data: dict, training: bool = False) -> dict:
        """
        处理单个batch的数据：
        1. 先调用父类的 simple_process 处理图像和数据样本（如果有）
        2. 对点云进行双视图体素化
        """
        # 让父类处理图像和数据样本（如果存在），但父类的体素化已被禁用
        data = super().simple_process(data, training)
        ...
        if 'points' in batch_inputs:
            points_list = batch_inputs['points']  # 列表，每个元素是一个样本的点云张量
            ...
            # 侧视图体素化
            side_voxels, side_coors, side_num_points = self._voxelize_points(
                points_list, self.side_voxel_layer)

            # 将结果添加到 batch_inputs 中，供后续模型使用
            batch_inputs['bev_voxels'] = bev_voxels
            batch_inputs['bev_num_points'] = bev_num_points
            batch_inputs['bev_coors'] = bev_coors
            batch_inputs['side_voxels'] = side_voxels
            batch_inputs['side_num_points'] = side_num_points
            batch_inputs['side_coors'] = side_coors
    ...
    def _voxelize_points(self,
        ...
        """
        对点云列表进行体素化，返回合并后的体素、坐标和每个体素的点数。
        voxel_layer 的 training 状态由 PyTorch 自动管理，内部会根据 self.training
        选择合适的 max_voxels（如果配置为元组）。
        """
        all_voxels, all_coors, all_num_points = [], [], []

        for batch_idx, points in enumerate(points_list):
            # 调用体素化层，返回 (voxels, coors, num_points)
            voxels, coors, num_points = voxel_layer(points)

            # 为坐标添加 batch 索引（放在第一列，与官方一致）
            batch_inds = torch.full((coors.shape[0], 1), batch_idx,
                                    dtype=coors.dtype, device=coors.device)
            coors = torch.cat([batch_inds, coors], dim=1)
