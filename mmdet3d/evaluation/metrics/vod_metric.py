...
from mmdet3d.evaluation.functional.kitti_utils.eval_vod import kitti_eval
...
class VODMetric(BaseMetric):
    """VOD evaluation metric.
    ...
    def __init__(self,
                 ...
                 pr_curve_prefix: Optional[str] = None) -> None:
        self.default_prefix = 'Kitti metric'
        super(VODMetric, self).__init__(
        ...
        self.pr_curve_prefix = pr_curve_prefix
            ...
            for i, annos in enumerate(data_annos):
                if len(annos['instances']) == 0:
                    ...
                    for instance in annos['instances']:
                        ...
                        kitti_annos['difficulty'].append(instance['difficulty'])
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            if data_sample.get('lidar_path', False):
                result['sample_id'] = int(osp.basename(data_sample['lidar_path']).split('.')[0])
            else:
                result['sample_id'] = int(osp.basename(data_sample['img_path']).split('.')[0])
    ...
    def compute_metrics(self, results: List[dict]) -> Dict[str, float]:
        ...
        if self.pr_curve_prefix is not None:
            import os
            import pickle
            import matplotlib.pyplot as plt
            SAVE_PATH = self.pr_curve_prefix
            DIFFICUALIES = ['easy', 'moderate', 'hard']
            ret = PR_detail_dict['3d']
            ...
            min_overlaps = min_overlaps[:, :, [0,1,2]] # (num_overlap, metric, class)

            OVERLAPS = min_overlaps[:, 1, :]

            for i, cls in enumerate(self.classes):
                for j, diff in enumerate(DIFFICUALIES):
                    for k, overlab in enumerate(OVERLAPS[:, i]):
                        fig = plt.figure(figsize=(12,8))
                        fig.set_dpi(300)
                        p = ret['precision'][i, j, k, :]
                        r = ret['recall'][i, j, k, :]

                        title = 'class:{}, diff:{}, overlap:{}'.format(cls, diff, overlab)
                        plt.title(title)
                        plt.xlim(0,1)
                        plt.ylim(0,1)
                        plt.scatter(np.linspace(0, 1, p.shape[0]), p)

                        plt.savefig(SAVE_PATH + '/' + title+'.png')
            with open(SAVE_PATH + '/' + 'raw_data.pkl', 'wb') as f:
                pickle.dump(ret, f)
        return metric_dict
    ...
    def kitti_evaluate(self,
        ...
        for name in results_dict:
            ...
            pr_detail_dict = kwargs.get('PR_detail_dict', None)
            custom_methods = [0, 3]
            eval_range_list = ['entire_area', 'roi']
            result_str = ''
            for custom_method, eval_range in zip(custom_methods, eval_range_list):
                ap_result_str, ap_dict_ = kitti_eval(
                    gt_annos, results_dict[name], classes, eval_types=eval_types, pr_detail_dict=pr_detail_dict, custom_method=custom_method)
                ap_dict_ = ap_dict_[eval_range]
                for ap_type, ap in ap_dict_.items():
                    ap_dict[f'{name}/{eval_range}/{ap_type}'] = float(f'{ap:.4f}')
                ap_dict[f'{name}/{eval_range}/map'] = float(f"{(ap_dict_['Car_3d_all'] + ap_dict_['Pedestrian_3d_all'] + ap_dict_['Cyclist_3d_all']) / 3:.4f}")
    ...
    def format_results(
        ...
        sample_id_list = [result['sample_id'] for result in results]
    ...
    def bbox2result_kitti(
            ...
            pklfile_prefix: Optional[str] = None,
            submission_prefix: Optional[str] = None,
            sample_id_list: List[int] = None) -> List[dict]:
        ...
        for idx, pred_dicts in enumerate(
            ...
            if submission_prefix is not None:
                if sample_id_list is not None:
                    curr_file = f'{submission_prefix}/{sample_id_list[idx]:06d}.txt'
                else:
