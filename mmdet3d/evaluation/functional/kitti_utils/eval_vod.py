...
def bev_box_overlap(boxes, q_boxes, criterion=-1):
    ...
    r_iou = rotate_iou_gpu_eval(boxes, q_boxes, criterion)
    return r_iou
...
def d3_box_overlap(boxes, q_boxes, criterion=-1):
    from .rotate_iou import rotate_iou_gpu_eval
    r_inc = rotate_iou_gpu_eval(boxes[:, [0, 2, 3, 5, 6]],
...
def kitti_eval(gt_annotations, dt_annotations, current_classes, eval_types=['bbox', 'bev', '3d'],pr_detail_dict=None, custom_method=0):
    if custom_method == 0:
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(1, 2):  # min_overlaps.shape[0]):
            ...
            if i == 1:
                ...
                ret_dict['%s_3d_all_strict' % class_to_name[curcls]] = mAP3d[
                    j, 0, 0]  # get j class, difficulty, first min_overlap
                ret_dict['%s_bev_all_strict' % class_to_name[curcls]] = mAPbev[
                    j, 0, 0]  # get j class, difficulty, first min_overlap
                
                ret_dict['%s_3d_all_r40' % class_to_name[curcls]] = mAP3d_R40[
                    j, 0, 1]  # get j class, difficulty, second min_overlap
                ret_dict['%s_bev_all_r40' % class_to_name[curcls]] = mAPbev_R40[
                    j, 0, 1]  # get j class, difficulty, second min_overlap
                
                ret_dict['%s_3d_all_r40_strict' % class_to_name[curcls]] = mAP3d_R40[
                    j, 0, 0]  # get j class, difficulty, first min_overlap
                ret_dict['%s_bev_all_r40_strict' % class_to_name[curcls]] = mAPbev_R40[
                    j, 0, 0]  # get j class, difficulty, first min_overlap

    result_str = "Results: \n" \
                    +f"{'Entire annotated area:' if custom_method==0 else 'Driving corridor area:'} \n" \
                    +f"Car: {ret_dict['Car_3d_all']} \n" \
                    +f"Pedestrian: {ret_dict['Pedestrian_3d_all']} \n" \
                    +f"Cyclist: {ret_dict['Cyclist_3d_all']} \n" \
                    +f"mAP: {(ret_dict['Car_3d_all'] + ret_dict['Pedestrian_3d_all'] + ret_dict['Cyclist_3d_all']) / 3} \n" \
                    +"\n" \
                    +f"Car strict: {ret_dict['Car_3d_all_strict']} \n" \
                    +f"Pedestrian strict: {ret_dict['Pedestrian_3d_all_strict']} \n" \
                    +f"Cyclist strict: {ret_dict['Cyclist_3d_all_strict']} \n" \
                    +f"mAP strict: {(ret_dict['Car_3d_all_strict'] + ret_dict['Pedestrian_3d_all_strict'] + ret_dict['Cyclist_3d_all_strict']) / 3} \n" \
                    +"\n" \
                    +f"Car R40: {ret_dict['Car_3d_all_r40']} \n" \
                    +f"Pedestrian R40: {ret_dict['Pedestrian_3d_all_r40']} \n" \
                    +f"Cyclist R40: {ret_dict['Cyclist_3d_all_r40']} \n" \
                    +f"mAP R40: {(ret_dict['Car_3d_all_r40'] + ret_dict['Pedestrian_3d_all_r40'] + ret_dict['Cyclist_3d_all_r40']) / 3} \n" \
                    +"\n" \
                    +f"Car strict R40: {ret_dict['Car_3d_all_r40_strict']} \n" \
                    +f"Pedestrian strict R40: {ret_dict['Pedestrian_3d_all_r40_strict']} \n" \
                    +f"Cyclist strict R40: {ret_dict['Cyclist_3d_all_r40_strict']} \n" \
                    +f"mAP strict R40: {(ret_dict['Car_3d_all_r40_strict'] + ret_dict['Pedestrian_3d_all_r40_strict'] + ret_dict['Cyclist_3d_all_r40_strict']) / 3} \n" \
                    +"\n"
    if custom_method == 0:
        return result_str, {'entire_area': ret_dict}
    elif custom_method == 3:
        return result_str, {'roi': ret_dict}
    else:
