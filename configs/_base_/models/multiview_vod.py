...
model = dict(
    ...
    voxel_encoder=dict(
        type='MultiViewSparseConvEncoder',
        # BEV视图参数
        bev_voxel_size=[0.32, 0.32, 4.0],
        bev_in_channels=5,                     # 点特征维度：x,y,z,reflectance,?（根据数据）
        bev_feat_channels=[64],                # PFN中间通道
        bev_out_channels=64,
        bev_num_conv_layers=2,
        # 侧视图参数
        side_voxel_size=[0.32, 51.2, 0.125],
        side_in_channels=5,
        side_feat_channels=[32],
        side_out_channels=32,
        side_num_conv_layers=2,
        # 公共参数
        point_cloud_range=[0, -25.6, -3, 51.2, 25.6, 2],
        conv_channels=[64, 64],                # 稀疏卷积每层输出通道
        d_model=128,                           # 融合模块中间维度
        n_heads=4,
        dropout=0.1,
