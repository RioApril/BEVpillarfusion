...
from tools.dataset_converters import vod_converter as vod
from tools.dataset_converters import tj4d_converter as tj4d
...
def vod_data_prep(root_path,
    ...
    Args:
        ...
        with_plane (bool, optional): Whether to use plane information.
            Default: False.
    """
    vod.create_kitti_info_file(root_path, info_prefix, with_plane)
    vod.create_reduced_point_cloud(root_path, info_prefix)

    info_train_path = osp.join(out_dir, f'{info_prefix}_infos_train.pkl')
    info_val_path = osp.join(out_dir, f'{info_prefix}_infos_val.pkl')
    info_trainval_path = osp.join(out_dir, f'{info_prefix}_infos_trainval.pkl')
    info_test_path = osp.join(out_dir, f'{info_prefix}_infos_test.pkl')
    update_pkl_infos('vod', out_dir=out_dir, pkl_path=info_train_path)
    update_pkl_infos('vod', out_dir=out_dir, pkl_path=info_val_path)
    update_pkl_infos('vod', out_dir=out_dir, pkl_path=info_trainval_path)
    update_pkl_infos('vod', out_dir=out_dir, pkl_path=info_test_path)

def vodlidar_data_prep(root_path,
                    info_prefix,
                    version,
                    out_dir,
                    with_plane=False):
    ...
    Args:
        ...
        with_plane (bool, optional): Whether to use plane information.
            Default: False.
    """
    vod.create_kitti_info_file(root_path, info_prefix, with_plane, num_features=7)
    vod.create_reduced_point_cloud(root_path, info_prefix, num_features=7)
...
def tj4d_data_prep(root_path,
    ...
    Args:
        ...
        with_plane (bool, optional): Whether to use plane information.
            Default: False.
    """
    tj4d.create_kitti_info_file(root_path, info_prefix, with_plane)
    ...
    info_trainval_path = osp.join(out_dir, f'{info_prefix}_infos_trainval.pkl')
    info_test_path = osp.join(out_dir, f'{info_prefix}_infos_test.pkl')
    update_pkl_infos('tj4d', out_dir=out_dir, pkl_path=info_train_path)
    update_pkl_infos('tj4d', out_dir=out_dir, pkl_path=info_val_path)
    update_pkl_infos('tj4d', out_dir=out_dir, pkl_path=info_trainval_path)
    #update_pkl_infos('tj4d', out_dir=out_dir, pkl_path=info_test_path)
