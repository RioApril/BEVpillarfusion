...
class BEVLoadMultiViewImageFromFiles(LoadMultiViewImageFromFiles):
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        #         np.float32)
        #     # cam2img_array = np.eye(4).astype(np.float32)
        #     # cam2img_array[:3, :] = np.array(cam_item['cam2img']).astype(np.float32)   # 关键修改
        #     # FIX
        ...
        for _, cam_item in results['images'].items():
            ...
            # 确保 lidar2cam 是 4x4
            if lidar2cam_array.shape == (3, 4):
                lidar2cam_full = np.eye(4, dtype=np.float32)
                lidar2cam_full[:3, :] = lidar2cam_array
                lidar2cam_array = lidar2cam_full
            elif lidar2cam_array.shape != (4, 4):
                raise ValueError(f'Unexpected shape for lidar2cam: {lidar2cam_array.shape}')
            ...
            cam2img_data = np.array(cam_item['cam2img']).astype(np.float32)
            if cam2img_data.shape == (3, 4):
                cam2img_array[:3, :] = cam2img_data
            elif cam2img_data.shape == (4, 4):
                cam2img_array = cam2img_data
            else:
                raise ValueError(f'Unexpected shape for cam2img: {cam2img_data.shape}')
            cam2img.append(cam2img_array)
            lidar2img.append(cam2img_array @ lidar2cam_array)
