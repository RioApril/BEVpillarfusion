...
class ConvFuser(nn.Sequential):
    def __init__(self, 
                 ...
                 cut_img: bool = False,
                 img_channel: Optional[int] = None):
        """
        Args:
            in_channels: [img_channels, lidar_channels]
            out_channels: 融合后输出通道数
            init_mode: 'normal' / 'lidar_only' / 'lidar_direct' / 'img_direct'
                - 'lidar_direct': 直接返回点云特征，不融合（用于测试纯点云性能）
                - 'img_direct':   直接返回图像特征，不融合（用于测试纯图像性能）
            cut_img: 是否对图像特征进行通道降维
            img_channel: 降维后的图像通道数（cut_img=True时必须提供）
        """
        # 处理 direct 模式：不创建任何融合层，直接透传对应特征
        if init_mode == 'lidar_direct':
            super().__init__(nn.Identity())
            self.lidar_direct = True
            self.img_direct = False
            self.in_channels = in_channels
            self.out_channels = out_channels
        ...
        if init_mode == 'img_direct':
            super().__init__(nn.Identity())
            self.lidar_direct = False
            self.img_direct = True
            self.in_channels = in_channels
            self.out_channels = out_channels
            self.cut_img = cut_img
            self.img_channel = img_channel
        ...
        # 正常模式（normal 或 lidar_only）
        self.lidar_direct = False
        self.img_direct = False

        if cut_img:
            assert img_channel is not None, "img_channel must be provided when cut_img=True"
            assert len(in_channels) == 2, "in_channels must be [img_ch, lidar_ch] when cut_img=True"
            # 图像降维层（1x1 卷积）
            self.img_reduce = nn.Conv2d(in_channels[0], img_channel, kernel_size=1, bias=False)
            # 融合卷积的输入通道 = 降维后图像通道 + 原始点云通道
            fuse_in_channels = img_channel + in_channels[1]
        else:
            self.img_reduce = None
            fuse_in_channels = sum(in_channels)
            img_channel = in_channels[0]  # 仅用于后续初始化

        super().__init__(
            nn.Conv2d(fuse_in_channels, out_channels, 3, padding=1, bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(True),
        )
        self.in_channels = in_channels
        ...
        if init_mode == 'lidar_only':
            self._init_lidar_only()

    def _init_lidar_only(self):
        """将融合卷积初始化为忽略图像输入、点云恒等映射"""
        conv = self[0]   # Conv2d
        bn = self[1]     # BatchNorm2d

        # 确定点云通道在拼接后的起始索引
        if self.cut_img:
            lidar_start = self.img_channel
        else:
            lidar_start = self.in_channels[0]  # 原始图像通道数

        c_lidar = self.in_channels[1]
        weight = conv.weight.data  # shape: (out_channels, total_in, 3, 3)
        weight.zero_()

        # 为每个点云输出通道设置恒等映射（中心像素为1）
        for out_c in range(min(self.out_channels, c_lidar)):
            weight[out_c, lidar_start + out_c, 1, 1] = 1.0
    ...
    def forward(self, inputs: List[torch.Tensor]) -> torch.Tensor:
        # inputs 顺序约定：[img_feat, lidar_feat]
        img_feat, lidar_feat = inputs[0], inputs[1]

        # direct 模式：直接返回对应特征，不做融合
        if self.img_direct:
            return img_feat
        if self.lidar_direct:
            return lidar_feat

        if self.cut_img:
            img_feat = self.img_reduce(img_feat)

        # 拼接后送入卷积
        return super().forward(torch.cat([img_feat, lidar_feat], dim=1))
...
class TransFusionHead(nn.Module):
    ...
    def __init__(
        ...
        self.code_size = bbox_coder.pop('code_size')   # 从 bbox_coder 获取，应该是 10
        self.with_vel = (self.code_size == 10)       # 如果 code_size=10 表示有速度
        # add by rufish
    ...
    def forward_single(self, inputs, metas):
        ...
        # 获取当前特征图的空间尺寸 (H, W)
        feat_h, feat_w = fusion_feat.shape[-2:]

        # 原始 bev_pos 的形状: [1, H_bev*W_bev, 2] (因为 self.bev_pos 是 [1, H*W, 2])
        # 需要将其上采样到 (feat_h, feat_w)
        bev_pos = self.bev_pos.repeat(batch_size, 1, 1).to(fusion_feat.device)  # [B, H_bev*W_bev, 2]

        # 如果 bev_pos 的网格尺寸与当前特征图不一致，则进行上采样
        if int(np.sqrt(bev_pos.shape[1])) != feat_h:
            # 将 bev_pos 重塑为 (B, H_bev, W_bev, 2)
            bev_h = int(np.sqrt(bev_pos.shape[1]))
            bev_w = bev_h  # 假设是正方形网格，通常 grid_size[0]==grid_size[1]
            bev_pos = bev_pos.view(batch_size, bev_h, bev_w, 2).permute(0, 3, 1, 2)  # (B, 2, H_bev, W_bev)
            # 上采样到 (feat_h, feat_w)
            bev_pos = F.interpolate(bev_pos, size=(feat_h, feat_w), mode='bilinear', align_corners=False)
            # 转换回 (B, feat_h*feat_w, 2)
            bev_pos = bev_pos.permute(0, 2, 3, 1).reshape(batch_size, -1, 2)
        ...
        elif self.test_cfg['dataset'] == 'VOD':  # for Pedestrian & Cyclist in VOD
    ...
    def predict_by_feat(self,
        ...
        for layer_id, preds_dict in enumerate(preds_dicts):
            ...
            if self.test_cfg['dataset'] == 'nuScenes':
                ...
                self.tasks=[
                    dict(num_class=1, class_names=['Pedestrian'], indices=[0], radius=0.7),
                    dict(num_class=1, class_names=['Cyclist'], indices=[1], radius=0.7),
                    dict(num_class=1, class_names=['Car'], indices=[2], radius=0.7),
                ],
    ...
    def get_targets_single(self, gt_instances_3d, preds_dict, batch_idx):
        ...
        bbox_targets = torch.zeros([num_proposals, len(self.train_cfg['code_weights'])
                                    ]).to(center.device)
        bbox_weights = torch.zeros([num_proposals, len(self.train_cfg['code_weights'])
                                    ]).to(center.device)
        ...
        if len(pos_inds) > 0:
            ...
            # 将 7 维转换为 8 维：拆分角度为 sin 和 cos
            if pos_bbox_targets.shape[-1] == 7:
                # 分离角度
                yaw = pos_bbox_targets[:, 6:7]   # (N, 1)
                sin_yaw = torch.sin(yaw)
                cos_yaw = torch.cos(yaw)
                # 重新组合：前6维不变 + sin + cos
                pos_bbox_targets = torch.cat([
                    pos_bbox_targets[:, :6],   # x, y, z, w, l, h
                    sin_yaw,
                    cos_yaw
                ], dim=-1)   # 现在形状 (N, 8)
                # add by rufish
        ...
        # print("gt_labels_3d unique values:", torch.unique(gt_labels_3d))
        # print("gt_labels_3d counts:", torch.bincount(gt_labels_3d))
        return (
            labels[None],
            label_weights[None],
            bbox_targets[None],
    ...
    def loss_by_feat(self, preds_dicts: Tuple[List[dict]],
        ...
        # )
        # 获取预测热力图的尺寸
        pred_heatmap = preds_dict['dense_heatmap']  # shape: (B, num_classes, H_pred, W_pred)
        pred_h, pred_w = pred_heatmap.shape[-2:]

        # 获取目标热力图的尺寸
        target_h, target_w = heatmap.shape[-2:]

        # 如果尺寸不匹配，将目标热力图上采样到预测尺寸
        if pred_h != target_h or pred_w != target_w:
            # heatmap 可能是 (B, num_classes, H, W) 或 (num_classes, H, W)
            # 确保是 4D 张量以便上采样
            if heatmap.dim() == 3:
                heatmap = heatmap.unsqueeze(0)  # 添加 batch 维度
            heatmap = F.interpolate(heatmap, size=(pred_h, pred_w), mode='nearest')
            # 如果原本没有 batch 维度，可以 squeeze 回去，但通常保持 batch 维度也无妨
