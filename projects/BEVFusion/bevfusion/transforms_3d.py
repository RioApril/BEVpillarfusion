...
@TRANSFORMS.register_module()
class BEVFusionRandomFlip3D:
    """Random flip 3D augmentation for BEVFusion.

    Compared with `RandomFlip3D`, this class directly records the lidar
    augmentation matrix in the `data`.

    Args:
        enable_vertical_flip (bool): Whether to enable vertical flip (X-axis flip).
            Defaults to True. Set to False to avoid flipping points out of the
            point cloud range when the X range is non-negative (e.g., [0, 50]).
    """

    def __init__(self, enable_vertical_flip: bool = True):
        self.enable_vertical_flip = enable_vertical_flip

    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        flip_horizontal = np.random.choice([0, 1])  # Y-axis flip
        flip_vertical = np.random.choice([0, 1]) if self.enable_vertical_flip else 0
...
# class BEVFusionGlobalRotScaleTrans(GlobalRotScaleTrans):
#     """Compared with `GlobalRotScaleTrans`, the augmentation order in this
#     class is configurable: rotation, translation, scaling (RTS) by default,
#     or rotation, scaling, translation (RST) if order='RST'."""
...
#         assert order in ['RTS', 'RST'], f"order must be 'RTS' or 'RST', got {order}"
#         self.order = order
...
#             input_dict['transformation_3d_flow'].extend(['R', 'T', 'S'])
#         else:  # order == 'RST'
#             # 与 GlobalRotScaleTrans 相同的顺序: 旋转 -> 缩放 -> 平移
#             self._rot_bbox_points(input_dict)
#             if 'pcd_scale_factor' not in input_dict:
#                 self._random_scale(input_dict)
#             self._scale_bbox_points(input_dict)
